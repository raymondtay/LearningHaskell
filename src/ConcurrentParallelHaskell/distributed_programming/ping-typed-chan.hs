...
master peers = do
  ...
  mapM_ monitor ps -- monitor the processes
  ...
  forM_ ports $ \port -> do -- attempt to receive the sent messages from the peers (assuming that none of the peers went down during that time.)
    _ <- receiveChan port
    return ()
...
To build: ghc -O2 ./ping-typed-chan.hs
To run: ./ping-typed-chan [slave 127.0.0.1|0.0.0.0] 44445 &
        ./ping-typed-chan [slave 127.0.0.1|0.0.0.0] 44446 &
        ./ping-typed-chan
