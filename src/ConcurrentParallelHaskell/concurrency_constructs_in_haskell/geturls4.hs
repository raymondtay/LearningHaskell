...
-- Putting a call to `mask` inside the `forkIO` isn't enough. There is still a
-- possibility that the exception might be thrown even before `mask` is called.
-- For this reason, `forkIO` specified to create a thread that inherits the
-- masking state of the parent thread. This means that we can create a thread
-- that is born in the masked state by wrapping the call to `forkIO` in a
-- `mask`.
