...
--
-- | Observation notes:
-- | (a) When examining the eventlog for a performance run, i guessed that
-- decryption was faster than encryption (you can make a guess by looking at
-- the operations on those 2 functions) and the graphs revealed that keeping
-- each pipeline stage running on a single core is good for locality 
--
...
encrypt n e s = mapS (B.pack . show . power e n . code) s

decrypt :: Integer -> Integer -> Stream ByteString -> Par (Stream ByteString)
decrypt n d s = mapS (B.pack . decode . power d n . integer) s
