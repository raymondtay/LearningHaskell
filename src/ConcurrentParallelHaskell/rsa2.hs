-- Source code from rwh, the module ByteStringCompat module is no longer
-- relevant for ghc 8.4.3
...
-- public domain.
-- 
-- Build: ghc -O2 -threaded -rtsopts -eventlog ./rsa1.hs
-- Run: ./rsa1 encrypt /usr/share/dict/words > /dev/null +RTS -s
--
-- observation:
-- was expecting the number of sparks to go down from 26,563 as compared to the
-- run by rsa1.hs but it wasn't the case; looks like my initial guess wasn't
-- the case .. ah well ;) 
-- so one thing that improved was that the memory pressure (comparing the
-- values on "maximum residency" has indeeded reduced)
--
-- one thing consistent throughout is that the total elapased time hasn't
-- changed siginificant across the 3 different implementations.
--
