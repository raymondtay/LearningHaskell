...
--
-- I like to reuse the structures created above to solve another fictitious
-- problem : converting values of this structure to a series of bits.
-- 
data Bit = Z | O deriving Show -- zero or one
compress :: TypeRepr a -> a -> [Bit]
compress RUnit U = [Z]
...
main = do
  ...
  putStrLn . show $ "Compress a Int: "  ++ (show $ compress RInt 5)
  putStrLn . show $ "Compress a Char: " ++ (show $ compress RChar 'Z')
  putStrLn . show $ "Compressing a List of Ints: " ++ show (compress  (RChoice RUnit (RCombo RInt (rList RInt))) (fromList intList))
  putStrLn . show $ "Compressing a Tree of Ints: " ++ show (compress  (rTree RInt) (intTree))
